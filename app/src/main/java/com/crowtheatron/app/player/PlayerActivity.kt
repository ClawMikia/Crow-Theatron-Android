...
class PlayerActivity : AppCompatActivity() {
        ...
        val gestureDetector = GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onScroll(e1: MotionEvent?, e2: MotionEvent, distX: Float, distY: Float): Boolean {
                ...
                if (abs(distX) > abs(distY)) {
                    ...
                    val seekDeltaMs = (-distX / screenWidth * dur * 0.3f).toLong(); val newPos = (exo.currentPosition + seekDeltaMs).coerceIn(0L, dur); exo.seekTo(newPos); showGestureHint(FormatUtils.formatDuration(newPos))
                } else {
                    if (e1nn.x / screenWidth < 0.5f) {
                        ...
                        val delta = -distY / binding.playerView.height; currentVolume = (currentVolume + delta * 0.5f).coerceIn(0f, 1f); player?.volume = currentVolume * working.audioBoost.coerceIn(1f, 2f); working = working.copy(volumeLevel = currentVolume)
    ...
    private fun bindTrimSeekers() {
        val full = max(metaDurationMs(), 1L); binding.seekTrimStart.progress = ((working.trimStartMs * 1000L) / full).toInt().coerceIn(0, 1000)
        binding.seekTrimEnd.progress = if (working.trimEndMs <= 0L) 1000 else ((working.trimEndMs * 1000L) / full).toInt().coerceIn(0, 1000)
        binding.tvTrimStart.text = "Start: ${FormatUtils.formatDuration(working.trimStartMs)}"; binding.tvTrimEnd.text = "End: ${FormatUtils.formatDuration(if (working.trimEndMs <= 0L) full else working.trimEndMs)}"
    }
    ...
    private fun setupEnhancementSpinner() {
        ...
        val ad = object : ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, labels) {
            override fun getView(pos: Int, convert: View?, parent: ViewGroup) = super.getView(pos, convert, parent).also { (it as? android.widget.TextView)?.setTextColor(resources.getColor(R.color.crow_on_bg, theme)) }
            override fun getDropDownView(pos: Int, convert: View?, parent: ViewGroup) = super.getDropDownView(pos, convert, parent).also { (it as? android.widget.TextView)?.apply { setTextColor(resources.getColor(R.color.crow_on_bg, theme)); setBackgroundColor(resources.getColor(R.color.crow_surface_elevated, theme)); setPadding(32, 24, 32, 24) } }
        }
    ...
    private fun applyPitchStep(progress: Int) { val semis = (progress - 6).coerceIn(-6, 6); binding.pitchLabel.text = pitchLabel(semis); working = working.copy(pitchSemitones = semis); applyPitchAndSpeed(semis, currentSpeed) }
    ...
    private fun enterPiP() {
        ...
        val videoSize = player?.videoSize; val rational = if (videoSize != null && videoSize.width > 0 && videoSize.height > 0) { val r = videoSize.width.toFloat() / videoSize.height.toFloat(); if (r < 0.4184f) Rational(4184, 10000) else if (r > 2.39f) Rational(239, 100) else Rational(videoSize.width, videoSize.height) } else { Rational(16, 9) }
